...
if not cfg.RESUME_EPOCH:
    ...
    if not cfg.model_name.startswith('efficientnet'):
        model = cfg.MODEL_NAMES[cfg.model_name](num_classes=cfg.NUM_CLASSES)  # 初始化模型
        # # 冻结前边一部分层不训练
        # ct = 0
...
for iteration in range(start_iter, max_iter):
    ...
    out = model(images)
    # print(out)  #     ///  grad_fn=<AddmmBackward>
    loss = criterion(out, labels)

    optimizer.zero_grad()  # 清空梯度信息，否则在每次进行反向传播时都会累加
    # print(loss)  # tensor(1.1341)   /// tensor(1.2752, grad_fn=<NllLossBackward>)
