    ...
    if not test:
        if LOCAL_PRETRAINED['resnet101'] == None:
            state_dict = load_state_dict_from_url(
                model_urls['resnet101'], progress=True)
        else:
...
def Resnext101_32x8d(num_classes, test=False):
    ...
    if not test:
        if LOCAL_PRETRAINED['resnext101_32x8d'] == None:
            state_dict = load_state_dict_from_url(
                model_urls['resnext101_32x8d'], progress=True)
        else:
            state_dict = state_dict = torch.load(
                LOCAL_PRETRAINED['resnext101_32x8d'])
        model.load_state_dict(state_dict)
    fc_features = model.fc.in_features
    model.fc = nn.Linear(fc_features, num_classes)
    return model
...
def Resnext101_32x16d(num_classes, test=False):
    ...
    if not test:
        if LOCAL_PRETRAINED['resnext101_32x16d'] == None:
            state_dict = load_state_dict_from_url(
                model_urls['resnext101_32x16d'], progress=True)
        else:
            state_dict = state_dict = torch.load(
                LOCAL_PRETRAINED['resnext101_32x16d'])
        model.load_state_dict(state_dict)
    fc_features = model.fc.in_features
    model.fc = nn.Linear(fc_features, num_classes)
    return model
...
def Resnext101_32x32d(num_classes, test=False):
    ...
    if not test:
        if LOCAL_PRETRAINED['resnext101_32x32d'] is None:
            state_dict = load_state_dict_from_url(
                model_urls['resnext101_32x32d'], progress=True)
        else:
            state_dict = state_dict = torch.load(
                LOCAL_PRETRAINED['resnext101_32x32d'])
        model.load_state_dict(state_dict)
    fc_features = model.fc.in_features
    model.fc = nn.Linear(fc_features, num_classes)
    return model
...
def Resnext101_32x48d(num_classes, test=False):
    ...
    if not test:
        if LOCAL_PRETRAINED['resnext101_32x48d'] == None:
            state_dict = load_state_dict_from_url(
                model_urls['resnext101_32x48d'], progress=True)
        else:
            state_dict = state_dict = torch.load(
                LOCAL_PRETRAINED['resnext101_32x48d'])
        model.load_state_dict(state_dict)
    fc_features = model.fc.in_features
    model.fc = nn.Linear(fc_features, num_classes)
    return model
